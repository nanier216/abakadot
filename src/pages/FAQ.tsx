...
function FAQItem({ q, a, isOpen, onToggle }: { q: string; a: string; isOpen: boolean; onToggle: () => void }) {
  return (
    <div className={`bg-white dark:bg-slate-900 rounded-2xl border transition-all duration-200 overflow-hidden ${isOpen ? 'border-brand-teal/30 shadow-md shadow-brand-teal/5' : 'border-gray-100 dark:border-slate-700 shadow-sm hover:border-gray-200 hover:shadow-md'}`}>
      <button
        onClick={onToggle}
      ...
      >
        <span className={`font-display font-bold text-lg leading-snug transition-colors ${isOpen ? 'text-brand-teal' : 'text-slate-900 dark:text-white'}`}>
          {q}
        </span>
        <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-all ${isOpen ? 'bg-brand-teal text-white rotate-180' : 'bg-slate-100 text-slate-500'}`}>
      ...
      <section className="bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-800 py-12 md:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl">
            <span className="inline-block text-brand-teal text-xs font-bold uppercase tracking-widest mb-4">FAQ</span>
            ...
            <p className="text-xl text-slate-600 leading-relaxed">
              Everything you need to know about PinDot — the device, the learning modes, and what happens inside a session.
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-slate-700 shadow-sm p-8 md:p-10 flex flex-col sm:flex-row items-center gap-6 justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-500 text-sm mt-0.5">Check out our pricing page or try the live dashboard demo.</p>
              </div>
            </div>
