...
const premiumPrices: Record<BillingPeriod, { price: string; period: string; savings?: string; per?: string }> = {
  '1mo': { price: '₱500', period: 'per month', per: '₱500/mo' },
  '6mo': { price: '₱2,700', period: 'every 6 months', savings: 'Save ₱300', per: '₱450/mo' },
  '1yr': { price: '₱5,400', period: 'per year', savings: 'Save ₱600', per: '₱450/mo' },
};
    ...
    <div className="bg-[#F8FAFC] dark:bg-slate-950">
      ...
      <section className="bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-800 py-12 md:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="inline-block text-brand-teal text-xs font-bold uppercase tracking-widest mb-4">Pricing</span>
          <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-slate-900 dark:text-white leading-tight mb-5">
            Affordable access to<br className="hidden sm:block" />{' '}
            <span className="text-brand-teal">tactile literacy.</span>
          </h1>
          <p className="text-xl text-slate-600 max-w-2xl mx-auto leading-relaxed">
            One device. One mission. Transparent pricing designed for schools, families, and learners in low- and middle-income communities.
          </p>
        </div>
      </section>
      ...
      <section className="py-10 md:py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-10">
            <h2 className="font-display text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-1">Hardware</h2>
            <p className="text-slate-500">A one-time purchase — no ongoing hardware fees.</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
            <div className="grid md:grid-cols-2 gap-0">
              ...
              <div className="bg-gradient-to-br from-brand-teal/8 to-brand-orange/8 p-10 flex items-center justify-center min-h-[280px]">
                ...
                />
                <div className="text-center text-slate-400 font-medium hidden">PinDot Device</div>
              </div>
              ...
              <div className="p-8 md:p-10 flex flex-col justify-center">
                ...
                <h3 className="font-display text-3xl font-bold text-slate-900 dark:text-white mb-2">PinDot Device</h3>
                <p className="text-slate-500 mb-6 leading-relaxed">
                  The flagship AI-powered Braille learning device with a single-cell micro-solenoid mechanism, noise-cancellation microphone, and cloud connectivity — built for durability in any learning environment.
                </p>

                <div className="mb-8">
                  <div className="flex items-end gap-2">
                    <span className="font-display font-bold text-5xl text-slate-900 dark:text-white">₱11,000</span>
                    <span className="text-slate-500 mb-1.5">one-time</span>
                  </div>
                  <p className="text-slate-400 text-sm mt-1">Inclusive of device, charging cable, and setup guide</p>
                </div>
      ...
      <section className="pb-12 md:pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10">
            <div>
              <h2 className="font-display text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-1">Software Plans</h2>
              <p className="text-slate-500">Choose how you want to subscribe. Cancel anytime.</p>
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-gray-200 dark:border-slate-700 p-8 shadow-sm flex flex-col">
              <div className="mb-6">
                <h3 className="font-display text-2xl font-bold text-slate-900 dark:text-white mb-1.5">Basic</h3>
                <p className="text-slate-500 text-sm">Get started with foundational Braille at no cost.</p>
              </div>

              <div className="mb-8">
                <div className="flex items-end gap-2">
                  <span className="font-display font-bold text-5xl text-slate-900 dark:text-white">Free</span>
                </div>
                <p className="text-slate-400 text-sm mt-1">Forever — no credit card required</p>
              </div>
              ...
              <div className="relative">
                <div className="flex items-start justify-between mb-6">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm">All three learning modes + AI personalization.</p>
                  </div>
                </div>

                <div className="mb-3">
                  <div className="flex items-end gap-2">
                    <span className="font-display font-bold text-5xl text-white">{premiumPrices[billing].price}</span>
                    <span className="text-slate-400 mb-1.5">{premiumPrices[billing].period}</span>
                  </div>
                  <div className="flex items-center gap-3 mt-1.5">
                    {premiumPrices[billing].per && billing !== '1mo' && (
                      <span className="text-brand-teal text-sm font-medium">{premiumPrices[billing].per}</span>
                    )}
                    {premiumPrices[billing].savings && (
                      <span className="inline-flex items-center gap-1 bg-brand-orange/20 text-brand-orange text-xs font-bold px-2.5 py-1 rounded-full">
                        <Zap className="w-3 h-3" /> {premiumPrices[billing].savings}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <p className="text-center text-slate-500 text-xs mt-4">Requires PinDot hardware to use all modes.</p>
              </div>
            </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-slate-500 text-sm">
              All plans include cloud sync, automatic software updates, and access to curriculum improvements at no extra charge.{' '}
              <button onClick={() => onNavigate('faq')} className="text-brand-teal font-medium hover:underline">Have questions? Read the FAQ →</button>
            </p>
          </div>
        </div>
      </section>
